...
{
    public static QueryFilter<LanguageModel> IdIs(int id) => Common.IdIs<LanguageModel>(id);

    public static QueryFilter<LanguageModel> IdIsIn(IEnumerable<int> ids) => Common.IdIsIn<LanguageModel>(ids);

    public static QueryFilter<LanguageModel> CodeIs(string code) => Create(language => language.Code == code);
