...
internal class ThenIncludableQueryableAdapter<T, TProp> : IThenIncludable<T, TProp>, IQueryableConvertible<T>
    where T : class
    where TProp : class
{
    private readonly IIncludableQueryable<T, TProp> _queryable;
    
    public ThenIncludableQueryableAdapter(IIncludableQueryable<T, TProp> queryable)
    {
    ...
    public IThenIncludable<T, TNextProp> IncludeMany<TNextProp>(Expression<Func<T, IEnumerable<TNextProp>>> selector)
        where TNextProp : class
    {
    ...
    public IThenIncludable<T, TNextProp> ThenMany<TNextProp>(Expression<Func<TProp, IEnumerable<TNextProp>>> selector)
        where TNextProp : class
    {
        return new ThenManyIncludableQueryableAdapter<T, TNextProp>(
            _queryable.ThenInclude(selector));
